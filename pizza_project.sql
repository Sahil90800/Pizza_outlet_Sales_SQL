...
select round(sum(order_details.quantity*pizzas.price),2) as totalSales from order_details
...
on  pizzas.pizza_id= order_details.pizza_id group by 1 order by 2 desc limit 5 ;

-- Join the necessary tables to find the total quantity of each pizza category ordered.

select pizza_types.category,sum(order_details.quantity) as quantity from  pizza_types join pizzas
on pizza_types.pizza_type_id=pizzas.pizza_type_id  join order_details 
on  pizzas.pizza_id= order_details.pizza_id group by 1 order by 2 desc  ;

-- Determine the distribution of orders by hour of the day.


select hour(order_time),count(order_id) from orders group by 1 order by 2 desc ;
...
(select orders.date,sum(order_details.quantity) as quantity from orders join order_details on
orders.order_id=order_details.order_id group by 1) as order_quantity;
...
round(sum(order_details.quantity*pizzas.price),2) as total_sales from order_details join pizzas 
on pizzas.pizza_id= order_details.pizza_id  )*100,2) as revenue from pizza_types join pizzas
on pizza_types.pizza_type_id=pizzas.pizza_type_id  join order_details 
on  pizzas.pizza_id= order_details.pizza_id  group by 1 order by revenue desc ;
...
on orders.order_id=order_details.order_id group by 1 ) as sales ;
...
on  pizzas.pizza_id= order_details.pizza_id  group by 1,2) as a)as b where rn<=3 ;
